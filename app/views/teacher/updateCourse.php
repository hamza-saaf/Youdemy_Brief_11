...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $course = $TeacherController->getCourseById($id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $courceInstens = new Course($id, $title, $description, $content_type, contenu_url: $content_url, enseignant_id: $enseignant_id, categorie_id: $categorie_id);
    $isValid = $validationInstent::courseValidation($courceInstens);
    if($isValid){
        $TeacherController->updateCourse($courceInstens, $selectedTags);
    }else{
        $course = $TeacherController->getCourseById($id);
    }
}
        ...
        <div class="px-4 py-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <span class="ml-4 text-xl font-semibold">Update Course</span>
                </div>
            </div>
        </div>
    </nav>
        ...
        <form class="space-y-6" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="title" type="text" required value="<?= $course['coursTitle'] ?>"
                        ...
                        <select required name="categorie_id"
                            ...
                            <?php if ($categoris): ?>
                                <?php foreach ($categoris as $categori): ?>
                                    <?php echo '<option selected value="' . $course['categorieId'] . '">' . $course['categorieTitel'] . '</option>'; ?>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea required rows="4" name="description" 
                            class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                            placeholder="Provide a detailed description of your course..."><?php echo $course['description'] ; ?></textarea>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center text-sm text-gray-600">
                                <input type="radio" <?php echo $course['contenu_type'] === 'video' ? 'checked' : ''; ?>
                                    name="content_type" value="video" class="mr-2" checked>
                                Video
                            </label>
                            <label class="flex items-center text-sm text-gray-600">
                                <input type="radio" <?php echo $course['contenu_type'] === 'document' ? 'checked' : ''; ?>
                                    name="content_type" value="document" class="mr-2">
                                Document
                            </label>
                            <label class="block text-sm font-medium text-gray-700 mb-1">
                            ...
                            <div
                                ...
                                <input type="text" name="content_url" value="<?= $course['contenu_url'] ?>"
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <?php $tagsCheked = explode(',', $course['tags']); ?>
                    <?php
                    if ($tags) {
                        foreach ($tags as $tag) {
                            ...
                            if ($isChecked) {
                                ...
                                echo '<span class="text-sm text-gray-700"># ' . $tag['title'] . '</span>';
                            } else {
